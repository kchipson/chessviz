...
int checkRook(string move, bool color)
{
    ...
    if (!(((x==x_end)&&(y!=y_end))||((x!=x_end)&&(y==y_end))))
        { //Проверка на правильность хода
            cout << "Ошибка в ходе "<<move<<"! Ладья ходит по вертикали или горизонтали!" << endl;
            return 1;
        }
    ...
    if (x!=x_end){
        for(temp_x=x+z_x;temp_x!=x_end;temp_x=temp_x+z_x){
            if(board[temp_y][temp_x]!=' '){
                cout << "Ход "<<move<<" невозможен, т.к на пути хода ладьи стоит фигура "
                << endl;
                return 1;
            }
        }
...
int checkBishop(string move, bool color)
{
    ...
    if (!(abs(x-x_end)==abs(y-y_end)))
        { //Проверка на правильность хода
            cout << "Ошибка в ходе "<<move<<"! Слон ходит по диагонали!" << endl;
            return 1;
        }
    ...
    for(temp_x=x+z_x,temp_y=y+z_y;temp_x!=x_end;temp_x=temp_x+z_x,temp_y=temp_y+z_y)
        if(board[temp_y][temp_x]!=' '){
            cout << "Ход "<<move<<" невозможен, т.к на пути хода слона стоит фигура "
            << endl;
            return 1;
        }
